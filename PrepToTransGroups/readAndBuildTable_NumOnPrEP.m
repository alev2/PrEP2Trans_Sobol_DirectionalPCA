...
%        'Range',strcat(Alphabet(1),num2str(1),':',Alphabet(1),num2str(size(incidenceOut,1)+1)));
    ...
    values_g1(:,k) = tableIn(:,6); % if you give another input to xlsread you can select columns in the file 
    values_g2(:,k) = tableIn(:,7);
    values_g3(:,k) = tableIn(:,8);
    values_g4(:,k) = tableIn(:,9);
