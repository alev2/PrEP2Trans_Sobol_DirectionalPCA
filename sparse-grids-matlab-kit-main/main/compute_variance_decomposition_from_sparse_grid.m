function [Sob_i,Tot_Sob_i,Mean,Var] = compute_variance_decomposition_from_sparse_grid(S,Sr,nodal_values,domain,flags)


% COMPUTE_VARIANCE_DECOMPOSITION_FROM_SPARSE_GRID decomposes the variance of a scalar-valued function f:R^N -> R  in two steps: 
...
%
% [SOB_I,TOT_SOB_I,MEAN,VAR] = COMPUTE_VARIANCE_DECOMPOSITION_FROM_SPARSE_GRID(S,SR,NODAL_VALUES,DOMAIN,FLAGS) takes the
%       same inputs of CONVERT_TO_MODAL (see step 1 above) and returns the Sobol indices of the function F. 
%
%       In details:
%       1) SOB_I is the principal variance index of the i-th variable, y_i; i.e., the isolated variance of F that
%          can be ascribed to y_i only
%       2) TOT_SOB_I is the total variance  of y_i; i.e., the total variance of F that can be ascribed
...
% now for each variable, sum all coefficients that have been marked as contributing to the Total cariance, 
VTi=(Fact_STi')*gpce_squared;
Tot_Sob_i=VTi;

% repeat for the principal  variance
